...
function LoadingRows({ count = 4 }: { count?: number }) {
  return <div className="loading-list">{Array.from({ length: count }, (_, index) => <div className="skeleton-row" key={index}><span /><span /><span /></div>)}</div>;
}
...
function AppShell() {
  const { user, gateway, setUser, toast } = useApp();
...
function LoginPage() {
  const { gateway, setUser, toast } = useApp();
  const navigate = useNavigate();
  const [username, setUsername] = useState(gateway.mode === "demo" ? "admin" : "");
  const [password, setPassword] = useState(gateway.mode === "demo" ? "admin123" : "");
...
function BookForm({ initial, onClose, onSaved }: { initial?: Book | null; onClose: () => void; onSaved: (payload: BookPayload, id?: number) => Promise<void> }) {
  const [form, setForm] = useState<BookPayload>({ title: initial?.title || "", author: initial?.author || "", isbn: initial?.isbn || "", publisher: initial?.publisher || "", price: initial?.price || "0.00", category: initial?.category || "", total_quantity: initial?.total_quantity || 1 });
...
function BookCard({ book, admin, onEdit, onDelete }: { book: Book; admin?: boolean; onEdit?: () => void; onDelete?: () => void }) {
  return <article className={cn("book-card", !book.is_active && "book-inactive")}><div className="book-cover"><span>{book.title.slice(0, 1)}</span><small>{book.category || "未分类"}</small></div><div className="book-card-body"><div className="book-card-top"><StatusPill tone={book.is_active ? (book.available_quantity ? "success" : "warning") : "neutral"}>{book.is_active ? (book.available_quantity ? `${book.available_quantity} 册可借` : "暂无库存") : "已下架"}</StatusPill><span className="book-code">{book.book_code}</span></div>{admin ? <h3>{book.title}</h3> : <h3><Link className="book-title-link" to={`/books/${book.id}`}>{book.title}</Link></h3>}<p>{book.author} · {book.publisher || "未填写出版社"}</p><div className="book-card-bottom"><span>ISBN {book.isbn}</span>{admin && <span className="row-actions"><button onClick={onEdit}>编辑</button><button onClick={onDelete} disabled={!book.is_active}>停用</button></span>}</div></div></article>;
}

function BooksPage() {
  const { gateway, version, reload, toast, user } = useApp(); const admin = user?.role === "ADMIN"; const location = useLocation(); const navigate = useNavigate(); const [data, setData] = useState<{ items: Book[]; total: number } | null>(null); const [error, setError] = useState<unknown>(null); const [criterion, setCriterion] = useState<"title" | "author" | "isbn" | "category" | "book_code">("title"); const [query, setQuery] = useState(""); const [editing, setEditing] = useState<Book | null | undefined>(undefined); const [showInactive, setShowInactive] = useState(admin); const [searching, setSearching] = useState(false);
  const load = useCallback(async () => { setSearching(true); setError(null); try { const response = await gateway.listBooks({ [criterion]: query || undefined, page_size: 100 }); setData(response); } catch (value) { setError(value); } finally { setSearching(false); } }, [gateway, criterion, query]);
  useEffect(() => { void load(); }, [load, version]);
  useEffect(() => { if (admin && new URLSearchParams(location.search).get("new") === "1") { setEditing(null); navigate(location.pathname, { replace: true }); } }, [admin, location.pathname, location.search, navigate]);
...
function BookDetailPage() {
  const { bookId } = useParams(); const { gateway, user, toast, reload, version } = useApp(); const [book, setBook] = useState<Book | null>(null); const [error, setError] = useState<unknown>(null); const [borrowing, setBorrowing] = useState(false);
  useEffect(() => { if (!bookId) return; gateway.getBook(Number(bookId)).then(setBook).catch(setError); }, [gateway, bookId, version]);
...
function ReadersPage() {
  const { gateway, version, reload, toast } = useApp(); const location = useLocation(); const navigate = useNavigate(); const [readers, setReaders] = useState<User[] | null>(null); const [error, setError] = useState<unknown>(null); const [editing, setEditing] = useState<User | null | undefined>(undefined); const [query, setQuery] = useState("");
  useEffect(() => { let alive = true; gateway.listReaders().then((value) => alive && setReaders(value)).catch((value) => alive && setError(value)); return () => { alive = false; }; }, [gateway, version]);
  useEffect(() => { if (new URLSearchParams(location.search).get("new") === "1") { setEditing(null); navigate(location.pathname, { replace: true }); } }, [location.pathname, location.search, navigate]);
...
function LoansPage() {
  const { gateway, user, version, reload, toast } = useApp(); const admin = user?.role === "ADMIN"; const [loans, setLoans] = useState<Loan[] | null>(null); const [readers, setReaders] = useState<User[]>([]); const [error, setError] = useState<unknown>(null); const [bookCode, setBookCode] = useState(""); const [readerId, setReaderId] = useState(""); const [borrowing, setBorrowing] = useState(false); const [filter, setFilter] = useState<"ALL" | "BORROWED" | "RETURNED" | "OVERDUE">("ALL");
  useEffect(() => { let alive = true; const requests: Promise<unknown>[] = [gateway.listLoans({ page_size: 100 })]; if (admin) requests.push(gateway.listReaders()); Promise.all(requests).then(([loanResponse, readerResponse]) => { if (!alive) return; setLoans((loanResponse as { items: Loan[] }).items); if (readerResponse) setReaders(readerResponse as User[]); }).catch((value) => alive && setError(value)); return () => { alive = false; }; }, [gateway, version, admin]);
  const filtered = (loans || []).filter((loan) => filter === "ALL" || filter === loan.status || (filter === "OVERDUE" && loan.status === "BORROWED" && loan.overdue_days > 0));
  const borrow = async (event: FormEvent) => { event.preventDefault(); setBorrowing(true); setError(null); try { await gateway.borrow(admin ? { book_code: bookCode, reader_id: Number(readerId) } : { book_code: bookCode }); setBookCode(""); toast("借阅记录已创建，借期为 30 天", "success"); reload(); } catch (value) { setError(value); } finally { setBorrowing(false); } };
  const returnLoan = async (loan: Loan) => { try { const preview: ReturnPreview = await gateway.returnPreview(loan.id); const detail = preview.overdue_days ? `\n逾期 ${preview.overdue_days} 天，罚款 ¥${preview.fine_amount}` : "\n本次无罚款"; if (!window.confirm(`确认归还《${preview.book_title}》吗？${detail}`)) return; await gateway.returnLoan(loan.id); toast("图书已归还，库存已恢复", "success"); reload(); } catch (value) { setError(value); } };
  const payFine = async (loan: Loan) => { if (!window.confirm(`确认登记 ${loan.loan_no} 的 ¥${loan.fine_amount} 罚款已缴吗？`)) return; try { await gateway.payFine(loan.id); toast("罚款状态已更新", "success"); reload(); } catch (value) { setError(value); } };
  return <><PageHeader eyebrow={admin ? "CIRCULATION / ADMIN" : "MY LOANS"} title={admin ? "借还书工作台" : "我的借阅"} detail={admin ? "办理借阅、确认归还并处理罚款状态。" : "查看当前借阅、归还记录和逾期情况。"} />{admin && <Panel className="borrow-panel" title="办理借阅" detail="输入图书编号并选择读者；后端会再次检查库存、借阅上限和逾期状态"><form className="inline-form" onSubmit={borrow}><label className="field"><span>图书编号 / ISBN</span><input value={bookCode} onChange={(event) => setBookCode(event.target.value)} placeholder="BK000001" required /></label><label className="field"><span>读者</span><select value={readerId} onChange={(event) => setReaderId(event.target.value)} required><option value="">选择读者</option>{readers.filter((reader) => reader.is_active).map((reader) => <option value={reader.id} key={reader.id}>{reader.name} · {reader.student_id}</option>)}</select></label><Button type="submit" disabled={borrowing}>{borrowing ? "办理中…" : "确认借阅"}</Button></form><p className="rule-note">借期 30 天 · 归还当日不计逾期 · 每逾期一天 0.10 元</p></Panel>}<ErrorBanner error={error} /><Panel className="filter-panel"><div className="filter-tabs">{[["ALL", "全部"], ["BORROWED", "借阅中"], ["OVERDUE", "逾期"], ["RETURNED", "已归还"]].map(([key, label]) => <button key={key} className={filter === key ? "filter-active" : ""} onClick={() => setFilter(key as typeof filter)}>{label}</button>)}</div></Panel>{!loans ? <LoadingRows count={5} /> : filtered.length ? <Panel className="table-panel"><LoanList loans={filtered} onReturn={(loan) => void returnLoan(loan)} onPayFine={(loan) => void payFine(loan)} /></Panel> : <EmptyState title="没有匹配的借阅记录" detail="切换筛选条件或办理一笔新的借阅。" />}</>;
}

function ProfilePage() {
  const { gateway, user, version } = useApp(); const [profile, setProfile] = useState<User | null>(null); const [error, setError] = useState<unknown>(null); useEffect(() => { if (!user) return; gateway.getReader(user.id).then(setProfile).catch(setError); }, [gateway, user, version]);
  return <><PageHeader eyebrow="ACCOUNT / PROFILE" title="个人资料" detail="账号信息由管理员维护。" /><ErrorBanner error={error} />{!profile ? <LoadingRows count={2} /> : <div className="profile-layout"><Panel className="profile-card"><div className="profile-hero"><span className="avatar avatar-large">{profile.name.slice(0, 1)}</span><div><h2>{profile.name}</h2><p>{profile.student_id}</p></div></div><dl className="detail-list"><div><dt>联系方式</dt><dd>{profile.contact || "—"}</dd></div><div><dt>借阅上限</dt><dd>{profile.borrow_limit} 本</dd></div><div><dt>账户状态</dt><dd><StatusPill tone={profile.is_active ? "success" : "neutral"}>{profile.is_active ? "正常" : "已注销"}</StatusPill></dd></div></dl></Panel><Panel title="借阅规则"><ul className="rule-list"><li><span>01</span>每次借阅期限为 30 个日历日。</li><li><span>02</span>存在逾期未还图书时，系统会阻止新的借阅。</li><li><span>03</span>逾期罚款按每天 0.10 元计算。</li></ul></Panel></div>}</>;
}
...
function AnalyticsPage() {
  const { gateway, version } = useApp(); const [days, setDays] = useState<7 | 30 | 90>(30); const [stats, setStats] = useState<DashboardStats | null>(null); const [data, setData] = useState<Analytics | null>(null); const [error, setError] = useState<unknown>(null); useEffect(() => { let alive = true; Promise.all([gateway.stats(), gateway.analytics(days)]).then(([nextStats, nextData]) => { if (alive) { setStats(nextStats); setData(nextData); } }).catch((value) => alive && setError(value)); return () => { alive = false; }; }, [gateway, days, version]); const maxBorrow = Math.max(1, ...(data?.daily_trends.map((item) => item.borrowed + item.returned) || [1])); const maxCategory = Math.max(1, ...(data?.category_distribution.map((item) => item.copy_count) || [1])); return <><PageHeader eyebrow="INSIGHTS / ANALYTICS" title="数据分析" detail="用借阅记录观察馆藏使用情况。" actions={<select className="period-select" value={days} onChange={(event) => setDays(Number(event.target.value) as 7 | 30 | 90)}><option value="7">最近 7 天</option><option value="30">最近 30 天</option><option value="90">最近 90 天</option></select>} /><ErrorBanner error={error} />{!stats || !data ? <LoadingRows count={4} /> : <><div className="stat-grid stat-grid-three"><StatCard label="借阅活跃度" value={stats.active_loans} note="当前在借" tone="blue" /><StatCard label="逾期借阅" value={stats.overdue_loans} note="需要跟进" tone="amber" /><StatCard label="可借册数" value={stats.available_copies} note={`共 ${stats.total_copies} 册`} tone="green" /></div><div className="analytics-grid"><Panel title="借还趋势" detail={`${formatDate(data.start_date)} — ${formatDate(data.end_date)}`}><div className="trend-chart" aria-label="借还趋势图">{data.daily_trends.map((item) => <div className="trend-column" key={item.date}><div className="trend-bars"><i style={{ height: `${Math.max(3, (item.borrowed / maxBorrow) * 100)}%` }} title={`借阅 ${item.borrowed}`} /><b style={{ height: `${Math.max(3, (item.returned / maxBorrow) * 100)}%` }} title={`归还 ${item.returned}`} /></div><small>{item.date.slice(5)}</small></div>)}</div><div className="chart-legend"><span><i className="legend-blue" />借阅</span><span><i className="legend-green" />归还</span></div></Panel><Panel title="分类馆藏" detail="按在用馆藏册数"><div className="bar-list">{data.category_distribution.map((item) => <div className="bar-row" key={item.category}><div><span>{item.category}</span><b>{item.copy_count} 册</b></div><span className="bar-track"><i style={{ width: `${(item.copy_count / maxCategory) * 100}%` }} /></span></div>)}</div></Panel><Panel title="热门图书" detail="所选期间借阅次数"><div className="rank-list">{data.popular_books.length ? data.popular_books.map((item, index) => <div className="rank-row" key={item.book_id}><strong>0{index + 1}</strong><span>{item.title}<small>{item.book_code}</small></span><b>{item.borrow_count} 次</b></div>) : <EmptyState title="期间没有借阅记录" />}</div></Panel><Panel title="逾期分布" detail="当前未归还记录"><div className="overdue-grid">{data.overdue_buckets.map((item, index) => <div key={item.label} className={index === 2 ? "overdue-cell overdue-high" : "overdue-cell"}><strong>{item.count}</strong><span>{item.label}</span></div>)}</div></Panel></div></>}</>;
}

function DataPage() {
  const { gateway, toast } = useApp(); const [file, setFile] = useState<File | null>(null); const [preview, setPreview] = useState<string[]>([]); const [result, setResult] = useState<FileImportResult | null>(null); const [error, setError] = useState<unknown>(null); const [loading, setLoading] = useState(false);
...
export default function App() {
  ...
  const context = useMemo(() => ({ gateway, user, setUser, version, reload, toast }), [gateway, user, version, reload, toast]);
  return <AppContext.Provider value={context}><HashRouter>{notice && <div className={`toast toast-${notice.tone}`} role="status">{notice.message}</div>}<Routes><Route path="/login" element={<LoginPage />} /><Route element={<AppShell />}><Route index element={<Navigate to={user?.role === "ADMIN" ? "/admin/overview" : "/catalog"} replace />} /><Route path="/admin/overview" element={<AdminGuard><OverviewPage /></AdminGuard>} /><Route path="/admin/books" element={<AdminGuard><BooksPage /></AdminGuard>} /><Route path="/admin/readers" element={<AdminGuard><ReadersPage /></AdminGuard>} /><Route path="/admin/loans" element={<AdminGuard><LoansPage /></AdminGuard>} /><Route path="/admin/analytics" element={<AdminGuard><AnalyticsPage /></AdminGuard>} /><Route path="/admin/data" element={<AdminGuard><DataPage /></AdminGuard>} /><Route path="/catalog" element={<ReaderGuard><ReaderLanding /></ReaderGuard>} /><Route path="/books/:bookId" element={<ReaderGuard><BookDetailPage /></ReaderGuard>} /><Route path="/my-loans" element={<ReaderGuard><LoansPage /></ReaderGuard>} /><Route path="/profile" element={<ReaderGuard><ProfilePage /></ReaderGuard>} /><Route path="/loans/:loanId/receipt" element={<ReceiptPage />} /></Route><Route path="*" element={<Navigate to={user ? (user.role === "ADMIN" ? "/admin/overview" : "/catalog") : "/login"} replace />} /></Routes></HashRouter></AppContext.Provider>;
}
